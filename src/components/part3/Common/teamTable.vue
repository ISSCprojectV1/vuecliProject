    ...
    <div class="dormitoryData" v-loading="loading" element-loading-text="加载中">
        <el-form :inline="true">
          ...
          <el-form-item>
                        ...
                        <el-button type="primary" @click="onClickQuery2">查询地图</el-button>
...
inject:['reload'],
  ...
  methods: {
    onClickQuery2(){
      ...
      var Management = [];
      var mapdata = [];
      getAllRegulators().then((res) => {
                console.log("传入数据api/part1/Multimodal-multigranularity" + res.data.data)
                chart.hideLoading();

                for(var i = 0; i < res.data.data.length;i++){
                    console.log(res.data.data[i].bourse)
                    Management.push(
                        {
                            "name":res.data.data[i].bourse,
...
function renderMap(map,data){
    ...
    onClickQuery1(){
      ...
      let echart1 = echarts.init(document.querySelector("#map"));
      const colors = [
        "#002AFF",
        "#B62AFF",
        "#95F300",
