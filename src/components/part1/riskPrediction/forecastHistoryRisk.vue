...
import {getForecastHistoryRisk} from "@/api/part1/riskPrediction";

export default {
  name: "forecastHistoryRisk",
  data() {
    return {
      value: '小麦',
      options: [],
  ...
  mounted() {
    ...
    getForecastHistoryRisk(this.value).then(res => {
      console.log("!")
  ...
  methods: {
    ...
    drawChartRiskPrediction() {
      ...
      for (let i = 0; i < this.formRisk.length;) {
        let j = i + 1;
        if (this.formRisk[i].risk === 1) {
          while (j < this.formRisk.length)
            if (this.formRisk[j].risk === 1)
              j++;
            else
              break;
          pieces.push({gte: i, lt: j, color: 'green'})
        } else if (this.formRisk[i].risk === 2) {
          while (j < this.formRisk.length)
            if (this.formRisk[j].risk===2)
              j++;
            else
              break;
